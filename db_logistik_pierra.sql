...
CREATE TABLE `transaction_products`  (
  ...
  INDEX `transaction_products_tp_product_code_foreign`(`tp_product_code` ASC) USING BTREE,
  INDEX `transaction_products_tp_storage_location_foreign`(`tp_storage_location` ASC) USING BTREE,
  CONSTRAINT `transaction_products_tp_product_code_foreign` FOREIGN KEY (`tp_product_code`) REFERENCES `products` (`product_code`) ON DELETE RESTRICT ON UPDATE RESTRICT,
  CONSTRAINT `transaction_products_tp_storage_location_foreign` FOREIGN KEY (`tp_storage_location`) REFERENCES `storages` (`storage_location`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci ROW_FORMAT = Dynamic;
